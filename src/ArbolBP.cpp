...
void ArbolBP::crear_nodo(int id_padre){
    ...
    if(op==1){
        ...
        std::cout<<"Que tamano tiene (MB)?: ";
        std::cin>>tamaño;
        while(tamaño<=0){
            std::cout<<"Ingrese tamano valido: ";
            std::cin>>tamaño;   
        }
        NodoArchivo* nuevo = new NodoArchivo(nuevoID(),tipo,tamaño);
        insertar_nodo_grafo(nuevo->getID(),nuevo);
        NodoGrafo* p = buscar_nodo_grafo(id_padre);
        NodoDirectorio* padre = static_cast<NodoDirectorio*>(p);
        padre->agregarHijo(nuevo->getID());
        nuevo->agregarPadre(id_padre);
        std::cout<<"NUEVO ARCHIVO-> ID:"<<nuevo->getID()<<std::endl;
    }else if (op==2){
        NodoDirectorio* nuevo = new NodoDirectorio(nuevoID());
        insertar_nodo_grafo(nuevo->getID(),nuevo);
        NodoDirectorio* padre = static_cast<NodoDirectorio*>(buscar_nodo_grafoSINTEXTO(id_padre));
        padre->agregarHijo(nuevo->getID());
        nuevo->agregarPadre(id_padre);
        std::cout<<"NUEVO DIRECTORIO-> ID:"<<nuevo->getID()<<std::endl;
    }else{
...
NodoGrafo* ArbolBP::buscar_nodo_grafoSINTEXTO(int clave){
if(raiz==nullptr) return nullptr;
    NodoBPlusBase* aux = raiz;
    int ES=1;
    while(!aux->hoja()){
...
void ArbolBP::listar_contenido(int id_directorio){
    ...
    if (hijos.empty()) {
        std::cout << "El directorio:" << id_directorio << " esta vacio." << std::endl;
        return;
    }
    for(int i = 0;i<hijos.size();i++){
        NodoGrafo* son = buscar_nodo_grafoSINTEXTO(hijos[i]);

        if(son->es_directorio){
            NodoDirectorio* soon = static_cast<NodoDirectorio*>(son);
            std::cout<<"Directorio:";
            soon->show();
        } else {
            NodoArchivo* soon = static_cast<NodoArchivo*>(son);
            std::cout<<"Archivo:";
            soon->show();
        }
        std::cout<<std::endl;
...
void ArbolBP::rutasREC(NodoGrafo* actual, std::string rutaParcial,
    ...
    std::vector<int>& padres = actual->lista_padres; 
    
    if (padres.empty()) {
        rutasFinales.push_back(rutaParcial);
        return;
    }
